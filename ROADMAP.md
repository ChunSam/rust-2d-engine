...
> 작성일: 2026-05-25 | 기준 버전: v0.45.0 (Phase 44 완료)  
...
**배경**: 현재 텍스트는 단일 폰트·크기·색상만 지원 — 다이얼로그·UI 퀄리티 병목. 또한 `TextInput`이 ASCII char 단위만 처리해 아시아권 출시 시 닉네임/채팅 입력 불가 → winit IME 조합 이벤트 필수(49c).
...
**배경**: 현재 `crate-type = ["cdylib", "rlib"]` + `wasm-pack` 빌드만 존재. 네이티브 게임을 itch.io/Steam에 올릴 패키징 경로가 전무 — "상용 배포 가능"(Milestone 2)의 미충족 핵심.
...
| 서브 | 기능 |
|------|------|
| 56a | GPU 파티클 (컴퓨트 셰이더, 현재 CPU 파티클 대체) |
...
| **v1.0.0** | Phase 59 완료. API freeze, crates.io 게시 |
